...
import { deleteUserById, followUserById, getAllUsers, getUser, login, register, updateUserById,google_login } from "../actions/user.actions";
...
const userSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(google_login.fulfilled, (state, action) => {
      // Add user to the state array
      state.userId = action.payload.userId;
      state.loginStatus = true;
      sessionStorage.setItem("userId", action.payload.userId);
    ...
    builder.addCase(google_login.rejected, (state, action) => {
      toast.error("OAUTH went wrong");
    });
